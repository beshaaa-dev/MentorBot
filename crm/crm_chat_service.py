...
async def send_audio_to_chat(
    ...
    """
    Send an audio message to AMoCRM chat.

    Args:
        audio_url: URL to audio file (AMoCRM Drive URL)
        contact_id: CRM contact ID
    ...
    if not scope_id:
        logger.error("[send_audio_to_chat] Failed to get scope_id")
        return False

    channel_secret = config.CRM_CHAT_CHANNEL_SECRET

    if not channel_secret:
        logger.error("[send_audio_to_chat] Missing channel_secret in config")
        return False
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with async_amo_crm_rate_limiter.limit():
                async with session.post(url, headers=headers, data=chat_request_body, timeout=aiohttp.ClientTimeout(total=30)) as response:
                    ...
                    if response.status not in (200, 201):
                        logger.error(
                            f"[send_audio_to_chat] Failed to create chat: {response.status} {text}"
                        )
                        return False
        ...
        if not chat_id:
            logger.error(f"[send_audio_to_chat] No chat ID in response: {result}")
            return False
        ...
        if not access_token:
            logger.error("[send_audio_to_chat] Failed to get access token!")
            return False
        ...
        async with aiohttp.ClientSession() as session:
            async with async_amo_crm_rate_limiter.limit():
                async with session.post(attach_url, headers=attach_headers, json=attach_body, timeout=aiohttp.ClientTimeout(total=30)) as attach_response:
                    ...
                    if attach_response.status not in (200, 201):
                        logger.warning(
                            f"[send_audio_to_chat] Failed to attach chat: {attach_response.status} {text}"
                        )

        # Step 3: Send audio message
        timestamp = int(time.time())
        msec_timestamp = int(time.time() * 1000)
        msgid = f"tgbot-audio-{msec_timestamp}"
        sender_id = f"tgbot-contact-{timestamp}"
        ...
        audio_request_body = json.dumps(audio_body, separators=(",", ":"))

        date = formatdate(timeval=None, localtime=False, usegmt=True)
        path = f"/v2/origin/custom/{scope_id}"

        content_md5 = hashlib.md5(audio_request_body.encode()).hexdigest()
        signature_string = "\n".join([method.upper(), content_md5, "application/json", date, path])
        signature = hmac.new(channel_secret.encode(), signature_string.encode(), hashlib.sha1).hexdigest()
        ...
        async with aiohttp.ClientSession() as session:
            async with async_amo_crm_rate_limiter.limit():
                async with session.post(url, headers=headers, data=audio_request_body, timeout=aiohttp.ClientTimeout(total=30)) as response:
                    text = await response.text()

                    if response.status in (200, 201):
                        return True
                    logger.error(
                        f"[send_audio_to_chat] Failed to send audio: HTTP {response.status} {text}"
                    )
                    return False

    except Exception as e:
        total_time = time.time() - start_time
        logger.error(f"[send_audio_to_chat] Exception occurred after {total_time:.2f}s: {e}", exc_info=True)
        return False
